//This program is designed for the multiple Poisson model, suitable for repeat, heterozygosity and coverage bias.
//Calculate ai values using the P(x) kmer species curve. 
...
int Ranks_within_genomic_freq = 1;  //1,standard; 2,hetero; 4 or 8, coverage bias.
...
{	cout << "estimate_repeat <reads_kmerfreq_file>" << endl;
	...
	cout << "   -z <int>  set rank number within one genomic frequency, default=" << Ranks_within_genomic_freq << endl;
...
void calculate_posterior_pi(int ai_num, qfloat *p_i, qfloat *new_p_i)
{
	...
	{	p_k[k] = 0.0;
		...
		{	
			qfloat c = (float(i+1)/Ranks_within_genomic_freq)*Mean_depth;
...
{	
	...
	while((c=getopt(argc, argv, "r:l:g:m:d:z:h")) !=-1) {
		switch(c) {
			...
			case 'z': Ranks_within_genomic_freq=atoi(optarg); break;
			case 'h': usage(); break;
			default: usage();
		}
	}
	...
	string str_z = boost::lexical_cast<string>(Ranks_within_genomic_freq);
	string genomic_frequency_distribution_file = reads_kmerfreq_file + ".z" + str_z + ".gfd";
	string coverage_depth_distribution_file = reads_kmerfreq_file + ".z" + str_z + ".cdd";
	...
	//int ai_num = (!Ranks_within_genomic_freq) ? Gfreq_range : Gfreq_range*2;
	int ai_num = Gfreq_range * Ranks_within_genomic_freq;
	...
	{	
		...
		{	
			...
			for (int i=0; i<ai_num; i++)
			{
				double out_i = float(i+1)/Ranks_within_genomic_freq;
